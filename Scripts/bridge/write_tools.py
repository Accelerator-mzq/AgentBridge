...
def _spawn_actor_python(level_path, actor_class, actor_name, transform, dry_run) -> dict:
    ...
    import unreal
    from ue_helpers import check_editor_ready
...
def _spawn_actor_rc(level_path, actor_class, actor_name, transform, dry_run) -> dict:
    ...
    # 生成 Actor（纳入 Undo 事务）
    spawn_resp = rc_spawn_actor_from_class(
...
def _set_actor_transform_python(actor_path, transform, dry_run) -> dict:
    from ue_helpers import find_actor_by_path, read_transform, check_editor_ready
    import unreal
    ...
    old_transform = read_transform(actor)
    ...
    actual_transform = read_transform(actor)
...
def _set_actor_transform_rc(actor_path, transform, dry_run) -> dict:
    from remote_control_client import get_property, set_property, check_connection
    ...
    # 读旧值
    old_loc = get_property(actor_path, "RelativeLocation").get("RelativeLocation", {})
    old_rot = get_property(actor_path, "RelativeRotation").get("RelativeRotation", {})
    old_scale = get_property(actor_path, "RelativeScale3D").get("RelativeScale3D", {})
    ...
    # 写新值（纳入 Undo）
    set_property(actor_path, "RelativeLocation",
                 {"X": loc[0], "Y": loc[1], "Z": loc[2]}, generate_transaction=True)
    set_property(actor_path, "RelativeRotation",
