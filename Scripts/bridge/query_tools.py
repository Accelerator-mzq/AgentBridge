...
三通道实现：
  ...
  Mock: 返回 Schemas/examples/ 中的预定义 JSON
...
def _get_current_project_state_rc() -> dict:
    from remote_control_client import call_function, SYSTEM_LIB, EDITOR_LEVEL_LIB
    version_resp = call_function(SYSTEM_LIB, "GetEngineVersion")
    engine_version = version_resp.get("ReturnValue", "")
    world_resp = call_function(EDITOR_LEVEL_LIB, "GetEditorWorld")
...
def _list_level_actors_rc(level_path=None, class_filter=None) -> dict:
    from remote_control_client import rc_get_all_level_actors
    resp = rc_get_all_level_actors()
...
def _get_actor_state_python(actor_path: str) -> dict:
    from ue_helpers import find_actor_by_path, read_transform, read_collision, read_tags
    import unreal
    ...
    return make_response(
        ...
        data={
            ...
            "transform": read_transform(actor),
            "collision": read_collision(actor),
            "tags": read_tags(actor),
        },
    )
...
def _get_actor_bounds_rc(actor_path: str) -> dict:
    ...
    resp = call_function(actor_path, "GetActorBounds", {"bOnlyCollidingComponents": False})
...
def _get_asset_metadata_rc(asset_path: str) -> dict:
    from remote_control_client import call_function, EDITOR_ASSET_LIB
    resp = call_function(EDITOR_ASSET_LIB, "DoesAssetExist", {"AssetPath": asset_path})
...
def _run_map_check_rc(level_path=None) -> dict:
    from remote_control_client import call_function, EDITOR_LEVEL_LIB
    call_function(EDITOR_LEVEL_LIB, "EditorExec", {"Command": "MAP CHECK"})
    return make_response(
