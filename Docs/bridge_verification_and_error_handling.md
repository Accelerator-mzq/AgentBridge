...
- Bridge 函数返回预定义的 mock 数据（即 `Schemas/examples/` 中的 example JSON）
...
| `Schemas/common/error.schema.json` | 错误对象的 Schema 定义 |
| `Schemas/common/primitives.schema.json` | status 枚举的 Schema 定义（唯一真理来源） |
