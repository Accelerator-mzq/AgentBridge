...
└── PythonPackage/agent_ue5/
    ├── bridge/
    │   ├── bridge_core.py           ← 通道切换 + call_cpp_plugin() + 统一响应
    ...
    │   └── uat_runner.py            ← L2 UAT Python 封装
    └── validators/
        └── validate_examples.py     ← Schema 校验真源

宿主项目侧仅保留薄包装入口，例如：
- `AgentTools/run_orchestrator.py`
...
C++ Plugin 的返回值通过 `FBridgeResponse::ToJsonString()` 序列化后，必须符合 `PythonPackage/agent_ue5/schemas/` 中定义的 JSON Schema。
...
**Schema 是编排层自有契约**——UE5 没有 JSON Schema 体系。Schema 校验真源保留在 `agent_ue5.validators.validate_examples`，宿主项目仓的 `Scripts/validation/validate_examples.py` 仅保留薄包装入口。
