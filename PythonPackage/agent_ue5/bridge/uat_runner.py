...
class UATRunner:
    ...
    def __init__(self, project_path: str = "", engine_dir: str = ""):
        ...
        """
        self.project_path = self._detect_project_path(project_path)
    ...
    def _execute_uat(self, args: str, sync: bool = True) -> UATRunResult:
        ...
        if not self.project_path:
            result.error_message = "Project .uproject path is empty. Use UE_PROJECT_PATH or pass project_path explicitly."
            return result
    ...
    def _detect_project_path(self, explicit_path: str) -> str:
        """自动检测 .uproject 路径。"""
        if explicit_path:
        ...
        env_path = os.environ.get("UE_PROJECT_PATH", "").strip()
        if env_path:
        ...
        for root in search_roots:
            ...
            while True:
                ...
                matches = sorted(current.glob("*.uproject"))
                if matches:
