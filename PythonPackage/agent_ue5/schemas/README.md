...
`PythonPackage/agent_ue5/schemas/` 是 AGENT + UE5 可操作层的数据协议层（Schema Repository）。
...
**v0.3 中 Schema 与 C++ Plugin 的关系**：C++ Plugin 中的 `FBridgeResponse::ToJsonString()` 输出的 JSON 必须通过 Schema 校验。Schema 是"契约"，C++ Plugin 是"实现"——两者通过 JSON 格式对齐。Schema 校验真源位于 `PythonPackage/agent_ue5/validators/validate_examples.py`，因为 UE5 C++ 没有 JSON Schema 验证库。
...
| `PythonPackage/agent_ue5/schemas/` | 工具输入输出的数据结构契约（编排层自有，UE5 无对应物） |
...
```
PythonPackage/agent_ue5/schemas/
├── README.md              # 本文件
├── common/                # 通用基础 Schema
...
所有 `$ref` 使用纯相对路径（相对于当前 Schema 文件位置），由 `PythonPackage/agent_ue5/validators/validate_examples.py` 的 RefResolver 基于文件系统路径解析。
...
`python -m agent_ue5.validators.validate_examples --strict` → 10 checked / 10 passed / 0 failed ✅
...
```bash
cd AgentUE5Framework
$env:PYTHONPATH="PythonPackage"
python -m agent_ue5.validators.validate_examples --strict
```
...
```bash
$env:PYTHONPATH="PythonPackage"
python -m agent_ue5.validators.validate_examples --example get_actor_state.example.json
```
...
```bash
$env:PYTHONPATH="PythonPackage"
python -m agent_ue5.validators.validate_examples --list
```
...
3. 在 `PythonPackage/agent_ue5/validators/validate_examples.py` 的 `EXAMPLE_TO_SCHEMA` 中增加映射
4. 运行 `python -m agent_ue5.validators.validate_examples --strict` 确认零错误
